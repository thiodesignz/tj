        ...
        <div className="flex flex-col gap-[40px] md:gap-[52px] lg:gap-[64px] items-start max-w-[1280px] w-full py-[60px] md:py-[80px] lg:py-[120px] px-[20px] lg:px-0">
          ...
          {posts.length === 0 && (
            <p className="font-[family-name:var(--font-geist)] text-[16px] theme-text-secondary py-[64px] text-center w-full">
              No blog posts yet. Add posts from the admin panel.
            </p>
          )}
          ...
          {featured && (
            <div className="flex flex-col lg:flex-row gap-[24px] w-full">
              ...
              <div className="flex flex-col gap-[24px] flex-1 min-w-0">
                {sideTop && (
                  <Link href={`/blog/${sideTop.slug}`} className="flex gap-[16px] cursor-pointer group">
                    ...
                    <div className="flex flex-col gap-[8px] flex-1 min-w-0">
                      <AuthorRow author={sideTop.author || "Thrihash"} date={formatDate(sideTop.createdAt)} category={sideTop.category || ""} />
                      <h3 className="font-[family-name:var(--font-geist)] font-semibold text-[17px] theme-text tracking-[-0.34px] leading-[22px] group-hover:text-primary transition-colors">{sideTop.title}</h3>
                      <p className="font-[family-name:var(--font-geist)] text-[14px] theme-text-secondary tracking-[-0.28px] leading-[20px] line-clamp-3">{sideTop.excerpt}</p>
                    </div>
                  </Link>
                )}
                {sideBottom && (
                  <Link href={`/blog/${sideBottom.slug}`} className="flex gap-[16px] cursor-pointer group">
                    <div className="theme-bg-card rounded-[16px] w-[160px] md:w-[200px] lg:w-[240px] h-[120px] md:h-[140px] lg:h-[160px] overflow-hidden shrink-0">
                    ...
                    <div className="flex flex-col gap-[8px] flex-1 min-w-0">
                      <AuthorRow author={sideBottom.author || "Thrihash"} date={formatDate(sideBottom.createdAt)} category={sideBottom.category || ""} />
                      <h3 className="font-[family-name:var(--font-geist)] font-semibold text-[17px] theme-text tracking-[-0.34px] leading-[22px] group-hover:text-primary transition-colors">{sideBottom.title}</h3>
                      <p className="font-[family-name:var(--font-geist)] text-[14px] theme-text-secondary tracking-[-0.28px] leading-[20px] line-clamp-3">{sideBottom.excerpt}</p>
                    </div>
                  </Link>
                )}
              </div>
          ...
          {grid.length > 0 && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-[24px] w-full">
              {grid.map((post) => (
                <Link key={post.id} href={`/blog/${post.slug}`} className="flex flex-col gap-[16px] cursor-pointer group">
                  ...
                  <AuthorRow author={post.author || "Thrihash"} date={formatDate(post.createdAt)} category={post.category || ""} />
                  <h3 className="font-[family-name:var(--font-geist)] font-semibold text-[17px] theme-text tracking-[-0.34px] leading-[22px] group-hover:text-primary transition-colors">{post.title}</h3>
                  <p className="font-[family-name:var(--font-geist)] text-[14px] theme-text-secondary tracking-[-0.28px] leading-[20px] line-clamp-2">{post.excerpt}</p>
                </Link>
              ))}
            </div>
          )}
